...
const startMessage =
  "Здравствуйте. Я AI-консультант по запуску AI-администраторов. Помогу понять, подойдет ли виртуальный оператор вашему бизнесу, расскажу про цены и помогу оставить заявку на бесплатный мини-аудит.";

const quickButtons = [
  ["Что умеет AI-администратор?", "Сколько стоит?"],
...
async function handleLeadMessage(bot, message, session, text) {
  ...
  if (telegramResult.success) {
    await bot.sendMessage(
      chatId,
      "Спасибо, заявка отправлена. Ильнур посмотрит ваш бизнес и подготовит пример сценария AI-администратора.",
      keyboardOptions()
    );
    return;
  }
